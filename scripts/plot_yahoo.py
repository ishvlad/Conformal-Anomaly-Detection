...
class PlotYahoo(object):
  ...
  def _setupDirectories(self):
    ...
    self.dataDir = os.path.abspath(os.path.join(root, "..", "data_yahoo"))
    self.labelsDir = os.path.abspath(os.path.join(root, "..", "labels"))
    self.resultsDir = os.path.abspath(os.path.join(root, "..", "result_yahoo"))


  def _getThresholds(self):
    thresholdsPath = os.path.join(self.configDir, "thresholds_yahoo.json")
  ...
  def _addLabels(self):
    ...
    labels = getJSONData(
      os.path.join(self.labelsDir, "yahoo_labels.json"))[self.dataFile]
...
if __name__ == "__main__":
  ...
  dataFiles = [
      # "A4_yahoo/A4Benchmark-TS11.csv"
      "A1_yahoo/real_1.csv"
  ]
  dataNames = [
      "Yahoo synthetic data"
  ]
  detectors=["numenta"]
  
  print len(dataFiles), len(dataNames)
  assert len(dataFiles) == len(dataNames)
